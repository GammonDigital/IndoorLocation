...
try:
    ...
    while True:
        beaconNumi = int(input('Please input starting beacon number for this batch (number must be an integer that does not start with 0): '))
        if beaconNumi in beaconNum:
            beaconNumi = int(input('Starting beacon number is already in the current registry, please select another starting beacon number: '))
        else:
            break
    while True:
        ...
        for dev in devices:
            if dev.rssi > beaconThres and not(dev.addr in beaconAddr) and dev.addr[:len(startingAddr)] == startingAddr: # Add to list only if the RSSI is larger than the threshold and the Addr is not already on the list
