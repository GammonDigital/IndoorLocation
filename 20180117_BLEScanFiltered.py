...
# Beacon id
beaconRegFile = open('/home/pi/beaconReg.csv')
...
# Output file
# outputFile = open('/home/pi/scanLog.csv', 'a', newline='')
...
class ScanDelegate(DefaultDelegate):
    ...
    def handleDiscovery(self, dev, isNewDev, isNewData):
        if isNewDev:
            ...
            with open('/home/pi/scanLog.csv', 'a') as f:
                f.write('{:%Y-%m-%d %H:%M:%S}, {}, {}\n'.format(datetime.datetime.now(), dev.addr, dev.rssi))
...
while True:
    ...
    devices = scanner.scan()  # Scans for 0.5 seconds
    for dev in devices:
        ...
        for i in range(len(beaconAddr)):
            if dev.addr == beaconAddr[i] and dev.rssi > beaconThres:
                mqttclient.publish("J3628",
                                   scannerId + ", " + "{:%Y-%m-%d %H:%M:%S}, ".format(datetime.datetime.now()) + str(
                                       beaconNum[i]) + ", " + str(dev.rssi))
